...
$this->title = $user['email'] . ' lekcijas';
...
<div class="grid-view">
    ...
    <?php
        foreach($PossibleThreeLectures as $lid){            
            ...
            ?>
            <p><?= Html::a('<span class="glyphicon glyphicon-plus"></span>' . $manualLectures[$lid], 
    ...
    <h3>Jaunā sarežģītības vērtība ir: <strong><?=$PossibleThreeLectures?></strong> <small>[netika atrasts neviens atbilstošs uzdevums]</small></h3>
    <?php } ?>
    <h3>Manuāla lekcijas piešķiršana:</h3>
    <?php $form = ActiveForm::begin(); ?>
    <?= $manualLectures ? $form->field($model, 'lecture_id')
        ->dropDownList(
            $manualLectures,           // Flat array ('id'=>'label')
            ['prompt'=>'']    // options
        ) : ' <p>Nav lekciju ko piešķirt</p>'?>   
    </div>
